...
n() {
    if command -v mmm &>/dev/null; then
        ...
        then
            wget -q --show-progress -O/tmp/mmm.tar.gz https://github.com/oranellis/mmm/releases/download/v0.3.1/mmm-linux-x86_64.tar.gz
            tar -xzvf /tmp/mmm.tar.gz -C /tmp
            mkdir -p ~/.local/bin
            cp /tmp/mmm ~/.local/bin
        elif [ "$(uname -m)" = "aarch64" ]
        then
            wget -q --show-progress -O/tmp/mmm.tar.gz https://github.com/oranellis/mmm/releases/download/v0.3.1/mmm-linux-aarch64.tar.gz
            tar -xzvf /tmp/mmm.tar.gz -C /tmp
            mkdir -p ~/.local/bin
            cp /tmp/mmm ~/.local/bin
        fi
